...
const ElectricBackground = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
    ...
    <svg className="w-full h-full opacity-100" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
      <defs>
 ...
 */
const SciFiCard = ({ icon: Icon, title, description, index, t }: { icon: any, title: string, description: string, index: number, t: (key: string) => string }) => {
  const [isVisible, setIsVisible] = useState(false);
...
export function LandingPage({ t: frameworkT, locale: frameworkLocale, setLocale: frameworkSetLocale }: { t?: (key: string) => string, locale?: 'en' | 'cn', setLocale?: (l: 'en' | 'cn') => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white selection:bg-cyber-green selection:text-black font-mono overflow-x-hidden">
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-black/90 backdrop-blur-xl border-b border-white/5 py-4' : 'py-8'}`}>
      ...
      <main className="relative z-10">
            ...
            <div className="relative hidden lg:block animate-[fade-in_1s_ease-out_0.3s_both]">
              ...
              <div className="relative border border-white/10 rounded-2xl overflow-hidden bg-black/60 backdrop-blur-3xl shadow-2xl">
                <div className="h-8 border-b border-white/10 bg-white/5 flex items-center px-4 gap-1.5">
                  ...
                  <div className="text-[8px] font-mono text-white/30 uppercase tracking-[0.2em]">{vt('grid_status')}</div>
                </div>
        ...
        <section id="core" className="px-6 py-12 md:px-12 max-w-7xl mx-auto border-t border-white/5 bg-[radial-gradient(circle_at_50%_0%,_rgba(0,255,157,0.05)_0%,_transparent_50%)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <SciFiCard icon={Cpu} title={vt('feat_1_title')} description={vt('feat_1_desc')} index={0} t={vt} />
            <SciFiCard icon={Brain} title={vt('feat_2_title')} description={vt('feat_2_desc')} index={1} t={vt} />
            <SciFiCard icon={Shield} title={vt('feat_3_title')} description={vt('feat_3_desc')} index={2} t={vt} />
            <SciFiCard icon={Activity} title={vt('feat_4_title')} description={vt('feat_4_desc')} index={3} t={vt} />
            <SciFiCard icon={Infinity} title={vt('feat_5_title')} description={vt('feat_5_desc')} index={4} t={vt} />
            <SciFiCard icon={Database} title={vt('feat_6_title')} description={vt('feat_6_desc')} index={5} t={vt} />
            <SciFiCard icon={Globe} title={vt('feat_7_title')} description={vt('feat_7_desc')} index={6} t={vt} />
            <SciFiCard icon={Zap} title={vt('feat_8_title')} description={vt('feat_8_desc')} index={7} t={vt} />
          </div>
        </section>
        ...
        <section className="relative px-6 py-12 md:px-12 max-w-7xl mx-auto overflow-hidden">
          ...
          <RevealSection id="cta" delay={150}>
            <div className="relative border border-white/10 bg-white/[0.03] rounded-3xl p-12 md:p-20 text-center overflow-hidden border-t-cyber-green/20">
              ...
              <h2 className="text-5xl md:text-7xl font-black mb-6 tracking-tighter">{vt('cta_title_1')} <br className="md:hidden" /> <span className="italic text-cyber-green">{vt('cta_title_2')}</span></h2>
