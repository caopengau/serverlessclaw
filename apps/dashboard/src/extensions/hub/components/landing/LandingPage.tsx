...
const ElectricBackground = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
    ...
    <svg className="w-full h-full opacity-40" viewBox="0 0 1000 1000" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
      <defs>
 ...
 */
const LocaleSwitcher = ({ locale, setLocale }: { locale: 'en' | 'cn', setLocale: (l: 'en' | 'cn') => void }) => {
 ...
 */
const SectionCurtain = ({ children, id, register, isRevealed }: { children: React.ReactNode, id: string, register: any, isRevealed: boolean }) => (
  <div 
    ref={register(id)}
 ...
 */
const SciFiCard = ({ icon: Icon, title, description, delay, t, isRevealed }: { icon: any, title: string, description: string, delay: string, t: (key: string) => string, isRevealed: boolean }) => (
  <div 
  ...
  >
    ...
    <div className="relative p-6 bg-black/40 backdrop-blur-xl rounded-[11px] h-full flex flex-col">
      ...
      <h3 className="text-lg font-bold text-white mb-2 tracking-tight group-hover:text-cyber-green transition-colors">{title}</h3>
    ...
    <div className="min-h-screen bg-[#050505] text-white selection:bg-cyber-green selection:text-black font-mono overflow-x-hidden">
      <style jsx global>{`
        @keyframes electric_flow {
          0% { stroke-dasharray: 0 1000; stroke-dashoffset: 200; }
          50% { stroke-dasharray: 400 600; stroke-dashoffset: -300; }
          100% { stroke-dasharray: 0 1000; stroke-dashoffset: -800; }
        }
        @keyframes fade-in {
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-black/80 backdrop-blur-md border-b border-white/5 py-4' : 'py-8'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-[11px] font-black uppercase tracking-[0.2em] text-white/60">
            <a href="#vision" className="hover:text-cyber-green transition-colors">{vt('nav_vision')}</a>
            <a href="#core" className="hover:text-cyber-green transition-colors">{vt('nav_core')}</a>
            <a href="#security" className="hover:text-cyber-green transition-colors">{vt('nav_security')}</a>
          </div>
      ...
      <section className="relative pt-32 pb-20 md:pt-48 md:pb-40 px-6 md:px-12 max-w-7xl mx-auto overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className={`relative z-10 space-y-8 transition-all duration-1000 ${isRevealed('hero-text') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-20'}`} ref={register('hero-text')}>
            ...
            <div className="flex items-center gap-8 pt-8 border-t border-white/5">
              <div>
                ...
                <div className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">{vt('uptime')}</div>
              </div>
              <div>
                <div className="text-2xl font-black text-white">~12ms</div>
                <div className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">{vt('latency')}</div>
              </div>
              <div>
                <div className="text-2xl font-black text-white">1.2M</div>
                <div className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">{vt('sync_rate')}</div>
              </div>
            </div>
          </div>

          <div className={`relative hidden lg:block transition-all duration-1000 ${isRevealed('hero-visual') ? 'opacity-100 scale-100' : 'opacity-0 scale-75 rotate-12'}`} ref={register('hero-visual')}>
            ...
            <div className="relative border border-white/10 rounded-2xl overflow-hidden bg-black/40 backdrop-blur-3xl shadow-2xl">
              <div className="h-8 border-b border-white/5 bg-white/5 flex items-center px-4 gap-1.5">
                ...
                <div className="text-[8px] font-mono text-white/20 uppercase tracking-[0.2em]">{vt('grid_status')}</div>
              </div>
      ...
      {/* Feature Grid */}
      <section id="core" className="px-6 py-24 md:px-12 max-w-7xl mx-auto border-t border-white/5 bg-[radial-gradient(circle_at_50%_0%,_rgba(0,255,157,0.03)_0%,_transparent_50%)]">
        <SectionCurtain id="features-header" register={register} isRevealed={isRevealed('features-header')}>
          <div className="mb-16 space-y-4">
            <div className="text-cyber-green text-[10px] font-black uppercase tracking-[0.3em]">{vt('core_engine_capabilities')}</div>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight">{vt('powered_by')}</h2>
          </div>
        </SectionCurtain>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" ref={register('features-grid')}>
          <SciFiCard
            icon={Cpu}
      ...
      <section className="relative px-6 py-24 md:px-12 max-w-7xl mx-auto overflow-hidden">
        ...
        <SectionCurtain id="cta" register={register} isRevealed={isRevealed('cta')}>
          <div className="relative border border-white/10 bg-white/[0.02] rounded-3xl p-12 md:p-24 text-center overflow-hidden">
            ...
            <h2 className="text-5xl md:text-7xl font-black mb-8 tracking-tighter">{vt('cta_title_1')} <br className="md:hidden" /> <span className="italic text-cyber-green">{vt('cta_title_2')}</span></h2>
      ...
      <footer className="px-6 py-12 md:px-12 max-w-7xl mx-auto border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-8 opacity-50">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-black uppercase tracking-widest">{vt('footer_system')}</span>
        </div>
        <div className="text-[10px] font-mono uppercase tracking-[0.3em]">
          {vt('footer_rights')}
        </div>
        <div className="flex gap-8 text-[10px] font-black uppercase tracking-widest">
          <a href="#" className="hover:text-cyber-green transition-colors">{vt('footer_tos')}</a>
          <a href="#" className="hover:text-cyber-green transition-colors">{vt('footer_privacy')}</a>
        </div>
      </footer>
    </div>
  );
