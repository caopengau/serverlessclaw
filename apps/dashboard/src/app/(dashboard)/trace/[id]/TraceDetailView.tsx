...
export default function TraceDetailView({ id, nodes }: TraceDetailViewProps) {
      ...
      <div className="flex flex-col items-center justify-center h-screen bg-[#0a0a0a]">
        ...
        <Link href="/trace" className="mt-4 flex items-center gap-2">
          <Typography variant="caption" color="primary" weight="bold" className="hover:underline">
            <ArrowLeft size={16} className="inline mr-1" /> {t('TRACE_RETURN_TO_BASE')}
          </Typography>
        </Link>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <PageHeader
        ...
        subtitleKey={rootNode.initialContext?.userText ?? t('TRACE_DEFAULT_SUBTITLE')}
        stats={
      ...
      <div className="space-y-12">
        ...
        <section>
          ...
          <PathVisualizer trace={{ ...rootNode, nodes } as any} />
        </section>
            ...
            <section key={node.nodeId} className="space-y-6">
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  {t('TRACE_STEP_LABEL').replace('{id}', node.nodeId)}{' '}
                  {node.parentId
                    ? `(${t('TRACE_PARENT_LABEL').replace('{id}', node.parentId.slice(0, 8))})`
                    : `(${t('TRACE_ROOT_LABEL')})`}
                </span>
                ...
                {(node.status === 'error' || node.status === 'failed') && (
                  <div className="flex gap-2">
                    ...
                    >
                      <RefreshCw size={10} className="mr-1" /> {t('TRACE_RETRY')}
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                    ...
                    >
                      <Wrench size={10} className="mr-1" /> {t('TRACE_FIX')}
                    </Button>
                  </div>
                )}
              </Typography>
                        ...
                        <div>
                          ...
                          >
                            {step.type === TRACE_TYPES.TOOL_CALL
                              ? `${t('TRACE_EXECUTING')} ${(step.content as ToolCallContent)?.toolName || (step.content as ToolCallContent)?.tool || ''}`
                              : step.type === TRACE_TYPES.TOOL_RESULT
                                ? `${t('TRACE_OBSERVATION')} ${(step.content as ToolResultContent)?.toolName || (step.content as ToolResultContent)?.tool || t('COMMON_TOOL')}`
                                : step.type === TRACE_TYPES.LLM_CALL
                                  ? t('TRACE_AGENT_REQUEST')
                                  : step.type === TRACE_TYPES.LLM_RESPONSE
                                    ? t('TRACE_AGENT_RESPONSE')
                                    : t('TRACE_ERROR_DETECTED')}
                      ...
                      (step.content.result as { images?: string[] }).images ? (
                        <div className="mb-4 space-y-4">
                          <div className="text-[10px] text-cyber-blue/60 uppercase font-bold tracking-widest flex items-center gap-2">
                            <LayoutGrid size={12} /> {t('TRACE_GENERATED_VISUALS')}
                          </div>
                      ...
                      {step.type === TRACE_TYPES.LLM_RESPONSE ? (
                        <div className="mb-4 p-4 bg-cyber-green/[0.03] border border-cyber-green/10 rounded">
                          <div className="text-[10px] text-cyber-green/60 uppercase font-bold mb-2 tracking-widest flex items-center gap-2">
                            <Bot size={12} /> {t('TRACE_GENERATED_RESPONSE')}
                          </div>
                          ...
                          <div className="text-xs text-white/90 leading-relaxed whitespace-pre-wrap font-mono">
                            {(() => {
                              const content = (step.content as LlmResponseContent).content;
                              if (!content) return '';
                              try {
