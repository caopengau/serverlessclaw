...
if (process.env.NEXT_PUBLIC_ACTIVE_EXTENSIONS) {
  ...
  // Use relative path if the extension is already copied into the dashboard (common in SST builds)
  // Otherwise use the absolute path (common in local development)
  const isLocalAppRelative = fullPath.includes(path.join(__dirname, 'src/extensions/project'));
  const importPath = isLocalAppRelative ? './project/index' : fullPath;
