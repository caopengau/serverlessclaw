...
if (process.env.NEXT_PUBLIC_ACTIVE_EXTENSIONS) {
  ...
  if (path.isAbsolute(rawPath)) {
    ...
    // 1. Try local app relative (if it starts with ./src or src/) - common in SST builds
    const localPath = path.resolve(__dirname, rawPath);
    // 2. Try workspace root (3 levels up) - common in local dev
    const rootPath = path.resolve(__dirname, '../../../', rawPath);

    fullPath = fs.existsSync(localPath) ? localPath : rootPath;

    // Fallback log if neither exists (though Next.js will error later)
    if (!fs.existsSync(fullPath)) {
      console.warn(
        `[NextConfig] WARNING: Extension path not found: ${rawPath}. Tried ${localPath} and ${rootPath}`
      );
    }
  }
  ...
  const activeContent = `import * as ext from '${fullPath}';\nexport const init = ext.init;\n`;
