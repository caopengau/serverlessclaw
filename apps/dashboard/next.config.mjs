...
if (process.env.NEXT_PUBLIC_ACTIVE_EXTENSIONS) {
  ...
  if (path.isAbsolute(rawPath)) {
    ...
    // If it looks like a local extension (src/extensions/project), force relative path
    // regardless of existsSync check to avoid race conditions during build
    if (rawPath.includes('src/extensions/project')) {
      fullPath = localPath;
    } else {
    ...
    if (!fs.existsSync(fullPath) && !rawPath.includes('src/extensions/project')) {
      console.warn(
  ...
  // Use relative path for the import if we are in the dashboard src
  const importPath = fullPath.includes('framework/apps/dashboard/src/extensions/project')
    ? './project/index'
    : fullPath;

  const activeContent = `/** Auto-generated by next.config.mjs */\nimport * as ext from '${importPath}';\nexport const init = ext.init;\n`;
