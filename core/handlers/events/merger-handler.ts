...
export async function handlePatchMerge(eventDetail: Record<string, unknown>): Promise<void> {
  ...
  // 1. Clone fresh trunk to a temp directory
  const mergeDir = path.join('/tmp', `merge-${traceId ?? 'unknown'}-${Date.now()}`);
  const { execSync } = await import('child_process');

  try {
    execSync(`git clone --depth=1 . "${mergeDir}"`, {
      encoding: 'utf-8',
      timeout: 60000,
    });
  } catch (error) {
    logger.error('Merger: Failed to clone repository:', error);
    await wakeupInitiator(
      userId,
      initiatorId,
      `MERGE_FAILED: Could not clone repository. Error: ${error instanceof Error ? error.message : String(error)}`,
      traceId,
      sessionId,
      1
    );
  ...
  if (appliedPatches.length > 0) {
    try {
      ...
      logger.info(`Merger: Uploaded merged code to S3 (${appliedPatches.length} patches applied).`);
