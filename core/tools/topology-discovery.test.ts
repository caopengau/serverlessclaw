...
import { INSPECT_TOPOLOGY, DISCOVER_PEERS, REGISTER_PEER } from './topology-discovery';
...
describe('DISCOVER_PEERS tool', () => {
  ...
  it('should list all enabled agents by default', async () => {
    const result = await DISCOVER_PEERS.execute({});
    const parsed = JSON.parse(result);
  ...
  it('should filter by category', async () => {
    const result = await DISCOVER_PEERS.execute({ category: 'system' });
    const parsed = JSON.parse(result);
  ...
  it('should filter by capability', async () => {
    const result = await DISCOVER_PEERS.execute({ capability: 'deployment' });
    const parsed = JSON.parse(result);
  ...
  it('should respect topologyType option', async () => {
    const result = await DISCOVER_PEERS.execute({ topologyType: 'hierarchy' });
    const parsed = JSON.parse(result);
...
describe('REGISTER_PEER tool', () => {
  ...
  it('should register a new peer connection', async () => {
    const result = await REGISTER_PEER.execute({
      sourceAgentId: 'coder',
