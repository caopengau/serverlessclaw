...
const covPath = path.resolve(
  process.argv[2] || path.join(process.cwd(), 'coverage', 'coverage-final.json')
);
...
const rows = [];
// find repository root by searching upward for package.json
let repoRoot = null;
let cur = path.dirname(path.resolve(covPath));
while (cur && cur !== path.parse(cur).root) {
  const maybe = path.join(cur, 'package.json');
...
console.error('entries=', Object.keys(json).length);
console.error('sampleKey=', Object.keys(json)[0]);
