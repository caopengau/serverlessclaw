...
import { mapProfileToResource, mapToolToResources } from './utils';
...
export function inferNodeEdges(nodes: TopologyNode[]): TopologyEdge[] {
  ...
  if (dashboardNode) {
    ...
    const githubNode = nodes.find((node) => node.id === 'github');
    const coderNode = nodes.find((node) => node.id === 'coder' || node.id === 'coderagent');
    ...
    const usersNode = nodes.find((node) => node.id === 'external_users');
    if (usersNode) {
      if (apiNode) {
        edges.push({
    ...
    const notifierNode = nodes.find(
      (node) => node.id === 'notifier' || node.id === INFRA_NODE_ID.NOTIFIER
    );
    const telegramNode = nodes.find((node) => node.id === INFRA_NODE_ID.TELEGRAM);
