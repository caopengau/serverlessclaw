...
export default function ResourcePreview({ component, onAction }: ResourcePreviewProps) {
  ...
  const getIcon = () => {
    switch (props.resourceType.toLowerCase()) {
      case 'lambda': return <Zap size={18} className="text-cyber-green" />;
      case 'dynamodb': return <Database size={18} className="text-cyber-green" />;
      case 's3': return <Cloud size={18} className="text-cyber-green" />;
      default: return <FileCode size={18} className="text-cyber-green" />;
    }
  };
