  ...
  return (
    <>
      ...
      <aside
        ...
        ${isCollapsed ? 'lg:w-16 p-3' : 'lg:w-64 p-6'}
      `}
      >
        {/* Header */}
        <div className={`flex items-center justify-between gap-3 mb-6 ${isCollapsed ? 'flex-col' : ''}`}>
          <Link href={ROUTES.HOME} className="flex items-center gap-3 group shrink-0">
            <div className="relative w-8 h-8 flex-shrink-0 rounded-sm overflow-hidden group-hover:scale-105 transition-transform">
              <Image
                src="/icon.png"
        ...
        {/* Navigation */}
        <nav className="flex-1 space-y-0.5 text-sm overflow-y-auto overflow-x-hidden custom-scrollbar">
          {navItems.map((item, idx) => {
            if (item.type === 'header') {
              if (isCollapsed) return <div key={idx} className="h-px bg-border/40 my-4 mx-2" />;
              return (
                <div
                  key={idx}
                  className="text-muted-foreground px-2 text-[9px] tracking-[0.2em] font-black mb-1 pt-3 first:pt-0 uppercase"
                >
                  {item.label}
                </div>
              );
            ...
            return (
              <Link
                ...
                } ${isCollapsed ? 'justify-center py-2.5' : 'justify-between px-3 py-1.5'}`}
              >
        ...
        {/* Footer */}
        <div className={`block pt-6 border-t border-border space-y-4 ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
          <Link
            href={ROUTES.SYSTEM_PULSE}
            className="block group/status w-full"
            title={isCollapsed ? t('SYSTEM_STATUS') : undefined}
          >
            <div className={`bg-foreground/5 rounded transition-colors cursor-pointer ${isCollapsed ? 'p-2 flex justify-center' : 'p-3'} group-hover/status:bg-foreground/10`}>
              {isCollapsed ? (
                ...
                <>
                  ...
                  <div className="text-[10px] text-cyber-green mt-1.5 flex items-center gap-2 font-bold uppercase">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-cyber-green opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-cyber-green"></span>
                    </span>
          ...
          {!isCollapsed ? (
            <div className="flex flex-col gap-3 px-1 pb-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3 text-muted-foreground">
                  <button onClick={() => setTheme('light')} className={`p-1 rounded hover:bg-foreground/5 transition-colors ${theme === 'light' ? 'text-cyber-green' : ''}`}>
                    <Sun size={12} />
                  </button>
                  <button onClick={() => setTheme('dark')} className={`p-1 rounded hover:bg-foreground/5 transition-colors ${theme === 'dark' ? 'text-cyber-green' : ''}`}>
                    <Moon size={12} />
                  </button>
                  <button onClick={() => setTheme('system')} className={`p-1 rounded hover:bg-foreground/5 transition-colors ${theme === 'system' ? 'text-cyber-green' : ''}`}>
                    <Monitor size={12} />
                  </button>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center gap-4 pb-2">
               <button 
                 ...
                 className="p-1.5 rounded-full bg-foreground/5 text-muted-foreground hover:text-cyber-green transition-colors"
               >
                 {theme === 'dark' ? <Sun size={14} /> : <Moon size={14} />}
               </button>
               <button 
                 onClick={() => setLocale(locale === 'en' ? 'cn' : 'en')}
                 className="text-[10px] font-mono font-bold text-muted-foreground hover:text-cyber-green transition-colors uppercase"
               >
                 {locale === 'en' ? 'CN' : 'EN'}
               </button>
            </div>
