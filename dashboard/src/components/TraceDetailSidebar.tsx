        ...
        {loading && !trace ? (
          <div className="flex flex-col items-center justify-center h-40 gap-4 opacity-50">
            ...
            <div className="text-center py-10">
                <Typography variant="body" color="muted">{error}</Typography>
                <Button variant="ghost" size="sm" onClick={() => window.location.reload()} className="mt-4">Retry</Button>
            </div>
        ) : trace ? (
          <>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <Badge variant={trace.status === 'completed' ? 'primary' : trace.status === 'error' ? 'danger' : 'warning'} className="uppercase font-black text-[9px] px-2 py-0.5">
                  {trace.status}
                </Badge>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Card variant="glass" padding="sm" className="bg-foreground/[0.02] border-white/5">
                    <Typography variant="mono" className="text-[9px] uppercase opacity-40 mb-1">Total Tokens</Typography>
                    <div className="flex items-center gap-2 text-cyber-green text-sm font-black">
                        <Zap size={14} /> {(trace.totalTokens || 0).toLocaleString()}
                    </div>
                </Card>
                <Card variant="glass" padding="sm" className="bg-foreground/[0.02] border-white/5">
                    <Typography variant="mono" className="text-[9px] uppercase opacity-40 mb-1">Latency</Typography>
            ...
            <section className="space-y-3 pt-6 border-t border-white/5">
              ...
              <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/5">
                 <div className="flex flex-col">
                    <Typography variant="mono" className="text-[11px] font-bold text-cyber-blue">{trace.agentId}</Typography>
                    <Typography variant="mono" className="text-[9px] opacity-40 uppercase">Assigned Agent</Typography>
                 </div>
                 <Badge variant="outline" className="text-[9px] border-white/10 uppercase font-bold text-white/50">
                 ...
                 {trace.steps?.map((step: TraceStep, i: number) => (
                    <div key={i} className="relative z-10 pl-10 group">
                        <div className={`
                            ...
                            ${step.type === TRACE_TYPES.TOOL_CALL ? 'bg-amber-400' : 
                              step.type === TRACE_TYPES.LLM_CALL ? 'bg-cyber-blue' : 
                              step.type === TRACE_TYPES.LLM_RESPONSE ? 'bg-cyber-green' : 'bg-white/20'}
                        `} />
                        <div className="bg-foreground/[0.03] border border-white/5 p-4 rounded-xl group-hover:bg-foreground/[0.06] transition-all group-hover:border-white/10">
                            <div className="flex items-center justify-between mb-2">
                                <Typography variant="mono" className="text-[10px] uppercase font-black tracking-widest opacity-60">
                            ...
                            <Typography variant="body" className="text-[11px] leading-relaxed text-foreground/80 line-clamp-3 italic">
                                {typeof step.content === 'string' ? step.content : JSON.stringify(step.content).substring(0, 100)}
                            </Typography>
          ...
          <div className="text-center py-20 opacity-20">
            <Typography variant="h3" uppercase>Select a neuron to examine</Typography>
          </div>
        )}
      </div>
