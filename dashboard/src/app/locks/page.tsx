  ...
  return (
    <main className="flex-1 overflow-y-auto p-10 space-y-10 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-orange-500/5 via-transparent to-transparent">
      <header className="flex justify-between items-end border-b border-white/5 pb-6">
        <div>
          <Typography variant="h2" weight="bold" color="white" glow className="tracking-tight !text-orange-500">SESSION_TRAFFIC</Typography>
          <Typography variant="body" color="white" className="mt-2 block opacity-80">Real-time distributed lock manager for session isolation.</Typography>
        </div>
        <div className="flex gap-4">
          <Card variant="glass" padding="sm" className="px-4 py-2 min-w-[120px]">
            <Typography variant="mono" color="white" className="mb-1 block opacity-90">Active Locks</Typography>
            <Typography variant="h3" weight="bold" className="text-orange-500">{locks.filter(l => !l.isExpired).length}</Typography>
          </Card>
          <Card variant="glass" padding="sm" className="px-4 py-2 min-w-[120px]">
            <Typography variant="mono" color="white" className="mb-1 block opacity-90">Zombie Locks</Typography>
            <Typography variant="h3" weight="bold" color="white">{locks.filter(l => l.isExpired).length}</Typography>
          </Card>
        </div>
      </header>
        ...
        <div className="grid gap-4">
          {locks.length > 0 ? (
                ...
                <div className="flex gap-6 items-center">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <Typography variant="caption" weight="bold" color="white">SESSION::{lock.lockId}</Typography>
                            {lock.isExpired && (
                                <Badge variant="danger" className="text-[9px] px-1.5 py-0.5">Expired</Badge>
                            )}
                        </div>
                        <div className="flex gap-6 mt-2">
                            <Typography variant="mono" color="white" className="flex items-center gap-1.5 text-[10px]">
            ...
            <Card variant="solid" padding="lg" className="h-48 flex flex-col items-center justify-center opacity-20 border-dashed">
                ...
                <Typography variant="body" weight="normal">All lanes clear // No active sessions</Typography>
                <Typography variant="caption" color="muted" className="mt-2 block">System is idle.</Typography>
            </Card>
          )}
        </div>
      </section>
