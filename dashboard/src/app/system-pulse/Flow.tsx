...
const nodeTypes = {
  ...
  infra: ({ data }: { data: FlowNodeData }) => (
    <div className="relative group transition-all duration-300 z-10 hover:z-50">
      <div
        className={`px-4 py-2 shadow-lg rounded-md bg-[#0a0a0a] border border-${THEME.COLORS.INTEL}/30 min-w-[150px] relative overflow-hidden`}
      >
        <div
  ...
  return (
    <div className="h-full w-full bg-[#050505] rounded-lg border border-white/5 relative overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-10 space-y-2 pointer-events-none">
        <div
          className={`flex items-center gap-2 px-3 py-1 bg-black/80 border border-${THEME.COLORS.PRIMARY}/30 rounded-full`}
        >
