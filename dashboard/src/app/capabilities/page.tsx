    ...
    <main className="flex-1 overflow-y-auto p-10 space-y-10 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-yellow-500/5 via-transparent to-transparent">
      <header className="flex justify-between items-end border-b border-white/5 pb-6">
        <div>
          ...
          <p className="text-white/40 text-sm mt-2 font-light">Real-time management of agent toolsets and autonomous permissions.</p>
        </div>
      </header>
                  ...
                  <label 
                    ...
                    className={`flex items-start gap-3 p-3 rounded border transition-all cursor-pointer group ${
                      isEnabled 
                        ...
                        : 'bg-white/[0.02] border-white/5 text-white/40 opacity-60 grayscale hover:grayscale-0 hover:opacity-100'
                    }`}
                  >
                    <input 
                      type="checkbox" 
      ...
      <div className="glass-card p-6 border-white/5 text-white/40 flex items-center gap-4">
        ...
        <p className="text-xs italic leading-relaxed">
          ...
          <span className="text-white/60 mx-1 font-mono">dispatch_task</span> from the Main agent or 
          <span className="text-white/60 mx-1 font-mono">file_write</span> from the Coder may cause severe system degradation.
        </p>
      </div>
    </main>
  );
