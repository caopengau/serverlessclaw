...
export default async function TraceDetailPage({
  ...
  if (!trace) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#0a0a0a] text-white/40">
        <ShieldAlert size={48} className="mb-4 opacity-20" />
        <h1 className="text-xl font-bold">TRACE_NOT_FOUND</h1>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-10 space-y-10 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-cyber-blue/5 via-transparent to-transparent">
      <header className="max-w-5xl mx-auto mb-10">
        <Link href="/" className="text-white/40 hover:text-cyber-green transition-colors flex items-center gap-2 text-xs mb-6 group">
          <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> BACK_TO_INTELLIGENCE
        </Link>
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className={`text-[10px] px-2 py-0.5 rounded-sm font-bold ${
                trace.status === 'completed' ? 'bg-cyber-green/20 text-cyber-green' : 'bg-yellow-500/20 text-yellow-500'
              }`}>
      ...
      <main className="max-w-5xl mx-auto space-y-12">
        {/* Visualizer Section */}
        <section>
          <h2 className="text-[10px] uppercase tracking-[0.2em] font-bold text-white/40 flex items-center gap-2 mb-4">
            <LayoutGrid size={14} className="text-cyber-green" /> Neural Path Visualizer
          </h2>
        ...
        <section className="space-y-6">
          <h2 className="text-[10px] uppercase tracking-[0.2em] font-bold text-white/40 flex items-center gap-2 mb-4">
            <Activity size={14} className="text-cyber-green" /> Execution Timeline
          </h2>
            ...
            <div key={step.stepId} className="glass-card border-white/5 overflow-hidden">
              <div className="p-4 flex items-center justify-between bg-white/[0.02]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-[10px] text-white/40 font-bold uppercase tracking-wider">{step.type}</div>
                    <div className="text-sm font-medium">
                      {step.type === 'tool_call' ? `Executing ${step.content.toolName}` : 
                       step.type === 'tool_result' ? 'Observation received' :
                       step.type === 'llm_call' ? 'LLM Synthesis' : 'Error detected'}
                    </div>
                  </div>
                </div>
        ...
        {trace.finalResponse && (
          <section className="mt-12">
            <h2 className="text-[10px] uppercase tracking-[0.2em] font-bold text-white/40 flex items-center gap-2 mb-4">
              <CheckCircle size={14} className="text-cyber-green" /> Final Output
            </h2>
